...
# 테스트
x_test_norm = min_max_normalize(x_test)  # 입력 데이터 정규화
y_pred_norm = net(torch.tensor(x_test_norm, dtype=torch.float32))


y_pred = (((y_pred_norm.detach().numpy() * (max(y_train) - min(y_train))) + min(y_train))/100) * (w1 + w2 + w3 + w4) + 50
y_pred_norm = y_pred_norm.detach().numpy()[0][0]
print('Input: {}, Predicted Output: {:.4f}, Expected Output: {:.4f}, Accuracy: {:.2f}%'
      .format(x_test, float(y_pred), x_test_output, 100 * (1 - abs(float(y_pred) - x_test_output) / x_test_output)))

print('Scaled Predicted Output:', y_pred_norm)
