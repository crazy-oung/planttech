...
# 정답인 콩나물 길이는 4개의 센서값의 합으로 결정됩니다.
# 높은 센서값일수록 길이가 길어지도록 만듭니다.
np.random.seed(1)
...
#y_pred = y_pred_norm * 7.5
#print(f"input: {x_test}, predicted output: {float(y_pred[0]):.4f}")
...
accuracy = np.abs((y_true - y_pred_norm) / y_true) * 100
