...
#[식물번호, 나이, 예상등급, [센서1], [센서2], [센서3], [센서4]]
def print_predict_state(new_data, num_plants):
    ...
    for plant_num in range(1, num_plants + 1):
        ...
        # 각 식물번호에서의 최대 나이
        max_age = max(plant_age_list)

        for row in new_data:
            if row[0] == plant_num and row[1] == max_age:
                state = row[2]

        print("state : ", state)
        state = round(state)
        if state == 0:
            print(plant_num, " 번 식물의 예상 등급 : 매우 나쁨")
        elif state == 1:
            print(plant_num, " 번 식물의 예상 등급 : 나쁨")
        elif state == 2:
            print(plant_num, " 번 식물의 예상 등급 : 보통")
        elif state == 3:
            print(plant_num, " 번 식물의 예상 등급 : 좋음")
        elif state == 4:
            print(plant_num, " 번 식물의 예상 등급 : 매우 좋음")
        else:
            print(plant_num, "번 식물의 예상 등급 : 오류!")

print_predict_state(new_avg_status, PLANT_NUM)
